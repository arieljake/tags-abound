...
{
	var userId = new mongo.ObjectID(user._id);
	...
	db.collection("v1_user").find({_id: userId}).toArray(function (err,items) {
		...
		if (items.length == 1)
		{
			...
			user.username = savedUsername; // can't change username with this method (have to assure uniqueness)
			delete user._id;

			db.collection("v1_user").updateById(userId, user, function (err,users)
			{
				callback(user);
			});
		}
