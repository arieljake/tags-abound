...
{
	db.collection("v1_submission").find({_id: id},{limit: 1}).toArray(function (err,items) {
		 callback(items[0]);
	});
};
...
module.exports.updateSubmission = function(db,id,submission,callback)
{
	...
	db.collection("v1_submission").updateById(id, submission, function (err,submissions)
	{
		callback(submission);
	});
};
